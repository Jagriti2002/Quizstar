...
def database():
    try:
        con=pymysql.connect(host="localhost",user="root",password="",database="QuizIsGame")
        cur=con.cursor()
        cur.execute("select * from point where User Answer=%s",user_answer)
        row=cur.fetchone()
        #print(row) 
        cur.execute("insert into point (Question,Answer,User Answer) values(%s,%s,%s)",
                        (indexes,
                        user_answer,
                        answers
                        ))       
